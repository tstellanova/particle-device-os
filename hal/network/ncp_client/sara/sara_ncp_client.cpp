...
int SaraNcpClient::getAppFirmwareVersion() {
    ...
    int retMajor = 0, tempMajor = 0;
    int retMinor = 0, tempMinor = 0;
    int r = resp.scanf("%*[^,],%*[A.]%d.%d", &tempMajor, &tempMinor);
    if (r == 2 && (resp.readResult() == AtResponse::OK)) {
        retMajor = tempMajor;
        retMinor = tempMinor;
    }
    LOG(TRACE, "App firmware: %d", retMajor * 100 + retMinor);
    return retMajor * 100 + retMinor;
}
